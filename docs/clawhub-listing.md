...
The old package `@dgr-ai-labs/openclaw-sandbox` published artifact is 0.1.0-beta.1 at 4cec6796a4f0a08a0c5f1d817d1e1f9321a333c7. The table below describes this branch's proposed README-derived values. The approved new identity is `@dgr-ai-labs/openclaw-dgr-gate`, version `0.1.0-beta.2`; publication remains a founder action. Do not treat the table as a statement that the registry already contains the new summary.
...
The README body is the confirmed long-description surface. Publish it whole, including own-tools/operator limits. There is no separate promotional excerpt. The opening principle is followed by the exact package summary; both retain the two-tool scope. Metadata below is maintained here, not as a redundant README block.

## Package metadata

Family: `code-plugin`. Proposed discovery topics: `["agent-safety","policy-enforcement","fail-closed","audit-trail","tool-gating"]`. Release tag: `beta`. These are release inputs, not assertions about the current registry. Display name, version, license and category in the table are quoted from the manifests; topics are the founder-approved discovery labels for the README mechanism, not additional capabilities.
...
| summary | `"A decision gate for two synthetic OpenClaw finance tools: payments require stored invoice evidence; both tools check policy, record decisions, and fail closed."` | Opening scope paragraph; package.json description |
| readme | `"README.md; sha256:2737afc6e684826613e7ce2a6a70bd79b635f859aaf2fa49624988b4c6c91bf6"` | Entire README, including opening hook, limits and scan qualifier |
| family | `"code-plugin"` | Listing release input; plugin implementation described in README |
| version | `"0.1.0-beta.2"` | Version badge and Install; both manifests and lockfile |
...
| distTags | `["beta"]` | Install beta qualifier; listing release input |
...
| Scan/validation display | Old beta.1 version clean/benign, old package pending; new artifact unscanned; README Registry scan | Separate registry values; not an author-set verdict |
| Install snippet | Generated from package name; README Install also supplies the explicit beta command | Old package latestVersion is null; new package default installability NOT ESTABLISHED |
...
The new identity must be available under the existing `dgr-ai-labs` namespace and the founder must have publisher authorization. The candidate needs integration, a preview tied to the final clean source commit, publication, new-version/package scan results, and a fresh pinned registry install. Old beta.1 remains available under its old identity; its scans and install evidence cannot establish readiness of beta.2. No new account, namespace, authentication or listing was created here. Unconfirmed optional fields remain unset.
