...
This table prepares `@dgr-ai-labs/openclaw-dgr-gate@0.1.0`, with distribution tag `latest`. It is not a claim of publication. The previous release `0.1.0-beta.2` remains a separate immutable historical artifact.
...
| readme | `"README.md; sha256:e6496399edbfc013b90736c8ed58c283e2c1e57024c7732d45b325effb7069a5"` | Entire README, including opening hook, limits and scan qualifier |
...
| Install snippet | Generated from package name; README Install leads with the plain command and supplies pinned 0.1.0 as an alternative | Unpinned 0.1.0 installation must be verified after publication with latest |
...
Run `npm run check:listing`. To regenerate after a reviewed README edit, run `npm run check:listing -- --write`, then run the check again. It rejects tagline, version, license, category, topics, badge and whole-README drift, plus a missing/empty/oversized icon or an icon absent from npm pack output. It does not authenticate or publish. Unknown registry string limits are not invented by the check.
...
This preparation needs review and merge to main, a tag bound to the final clean source, a successful preview, founder publication with `latest`, new-version/package scan evidence, and both default and pinned registry-install checks. No tag, push or publication is performed in this task. Historical beta scans do not establish results for 0.1.0. Unconfirmed optional fields remain unset.

## Default README resolution

The published beta.2 contains README.md, but the [unversioned README request](https://clawhub.ai/api/v1/packages/%40dgr-ai-labs%2Fopenclaw-dgr-gate/file?path=README.md&preview=1) returned 404 while the [beta.2-pinned request](https://clawhub.ai/api/v1/packages/%40dgr-ai-labs%2Fopenclaw-dgr-gate/file?path=README.md&preview=1&version=0.1.0-beta.2) returned 200. This was version selection against `latestVersion: null`, not a stale tag or missing README. Publishing the new 0.1.0 release with `latest` is the founder-selected local remedy: it intentionally establishes the default install version. Confirm the default README endpoint returns 200 and displays the released content afterward; do not claim the remedy succeeded before those checks. See RELEASING.md and the historical diagnosis.
