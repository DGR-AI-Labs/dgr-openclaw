...
```sh
npm pack
openclaw plugins install ./dgr-ai-labs-openclaw-dgr-gate-0.1.2.tgz --force --accept-capabilities
```

Use `--force` only for the local archive you reviewed. The archive filename above matches package.json. The pre-execution gate covers the two bundled demo tools only.
